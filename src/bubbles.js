...
function spriteArrive(sprites, idx, target) {
	...
	if (idx < SPRITE_COUNT) {
		const desired = [target[0] - sprites.px[idx], target[1] - sprites.py[idx]];

		let speed = SPRITE_MAX_SPEED;
		const dist = vec2.getMag(desired);

		if (dist < 100) {
			speed = map(dist, 0, 100, 0, SPRITE_MAX_SPEED);
		}

		vec2.setMag(desired, speed);
...
function spriteFlee(sprites, idx, target) {
	...
	if (idx < SPRITE_COUNT) {
		...
		if (dist < fleeDistance) {
			vec2.setMag(desired, SPRITE_MAX_SPEED);
			desired[0] *= -1;
			desired[1] *= -1;
